...
registerNode('anchorNode', {
  draw(cfg, group) {
    ...
    const field_groups = cfg.extra.field_groups || []
    field_groups.forEach((_group, index) => {
      height += subSize[1]
      group.addShape('rect', {
        attrs: {
          x: offsetX,
          y: offsetY + size[1] + index * subSize[1],
          width: subSize[0],
          height: subSize[1],
          fill: fieldsColor,
          parent: mainId
      ...
      group.addShape('text', {
        attrs: {
          x: offsetX + subSize[0] / 2,
          y: offsetY + size[1] + subSize[1] / 2 + index * subSize[1],
          textAlign: 'center',
          textBaseline: 'middle',
          text: _group.name,
          fill: '#000000',
    ...
    const operateType = store.getters['flow/operateType']
